		...
		<div id="isi_popup_dusun_<?= $key_dusun ?>" style="visibility: hidden;">
			<div id="content">
				<h5 id="firstHeading" class="firstHeading">
					<?= set_ucwords($wilayah) . " <span class='nama-dusun'>" . set_ucwords($dusun['dusun']) . "</span><br><span class='nama-desa'>" . set_ucwords($dusun['nama_desa']) . "</span>"; ?>
				</h5>
							...
							<ul class="dropdown-menu">
								<?php foreach ($list_lap_front as $key => $value) : ?>
									<li <?= jecho($lap, $key, 'class="active"'); ?>>
										<a href="<?= site_url("statistik_web/chart_gis_dusun/$key/" . underscore($dusun[dusun]). "?desaid=".$dusun[id_desa]); ?>" data-remote="false" data-toggle="modal" data-target="#modalBesar" data-title="Statistik Penduduk <?= set_ucwords($wilayah) . " " . set_ucwords($dusun['dusun']); ?>">
											<?= $value ?>
										</a>
									</li>
								<?php endforeach; ?>
