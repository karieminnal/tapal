        ...
        <div class="col-md-12">
          <div class="box box-info">
            <div class="box-header with-border">
              <a href="<?= site_url("{$this->controller}/dialog_cetak/".$cus) ?>" class="btn btn-social btn-flat bg-purple btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Cetak Laporan" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Cetak Laporan"><i class="fa fa-print "></i> Cetak</a>
              <a href="<?= site_url("{$this->controller}/dialog_unduh/".$cus) ?>" title="Unduh Laporan" class="btn btn-social btn-flat bg-navy btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Unduh Laporan" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Unduh Laporan"><i class="fa fa-download"></i> Unduh</a>
            </div>
            <div class="box-body">
              <div class="row">
                <div class="col-sm-12">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-2">
                      <input name="tahun" placeholder="Tahun" type="text" class="form-control input-sm required" value="<?= $tahun ?>" onkeypress="if (event.keyCode == 13){$('#'+'mainform').attr('action', '<?= site_url("laporan/bulan/".$cus) ?>');$('#'+'mainform').submit();}" /></input>
                    </div>
                    <label class="col-sm-2 col-md-1 control-label" for="tahun">Bulan</label>
                    <div class="col-sm-3 col-md-2">
                      <select class="form-control input-sm" name="bulan" onchange="formAction('mainform','<?= site_url('laporan/bulan/'.$cus) ?>')" width="100%">
                  ...
                  <div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
                    <div class="row">
                      <div class="col-sm-12">
                        <?php switch ($cus) {
                          case "kk_per_dusun" :
                            include("donjo-app/views/laporan/tabel_kk_per_dusun.php");
                            break;
                          case "pend_per_umur" :
                            include("donjo-app/views/laporan/tabel_pend_per_umur.php");
                            break;
                          case "detail_mutasi" :
                            include("donjo-app/views/laporan/tabel_detail_mutasi.php");
                            break;
                          default :
                            include("donjo-app/views/laporan/tabel_bulanan.php");
                        } ?>
                      </div>
                    </div>
                  </div>
