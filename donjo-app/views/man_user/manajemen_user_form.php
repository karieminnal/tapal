				...
				<div class="col-md-9">
					<div class="box box-primary">
						<div class="box-header with-border">
							<a href="<?=site_url()?>man_user" class="btn btn-info btn-sm"><i class="fa fa-arrow-circle-o-left"></i> Kembali Ke Manajemen Pengguna</a>
						</div>
						<div class="box-body">
							...
							<div class="form-group showpilihdesa <?php echo ($user['id_grup'] != '1' ? '' : 'hide') ?>">
								<label class="col-sm-3 control-label" for="group">Pilih Desa</label>
								<div class="col-sm-8">
									<select class="form-control input-sm required" id="pilihDesa" name="id_desa">
										<?php foreach ($listdesa as $item): ?>
											<option <?php selected($user['id_desa'], $item['id']); ?> value="<?= $item['id'] ?>"><?= $item['nama_desa'] ?> - <?= $item['nama_kabupaten'] ?></option>
										<?php endforeach ?>
									</select>
								</div>
							</div>
							...
							<div class="form-group">
								<label class="col-sm-3 control-label" for="password1">Kata Sandi Ulang</label>
								<div class="col-sm-8">
									<input id="password1" name="password1" class="form-control input-sm required pwdLengthNist_atau_kosong" type="password" placeholder="Kata Sandi Ulang" <?php if ($user): ?>value="radiisi"<?php endif ?> ></input>
								</div>
							</div>
							<div class="form-group">
...
<script>
	$('document').ready(function() {
		...
		$('#id_grup').change(function(){
			...
			if(thisVal == 1) {
				$('.showpilihdesa').addClass('hide');
			} else {
				$('.showpilihdesa').removeClass('hide');
			}
		});
	});
</script>
