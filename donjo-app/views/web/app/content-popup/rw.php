							...
							<ul class="dropdown-menu">
								<?php foreach ($list_lap_front as $key => $value) : ?>
									<li <?= jecho($lap, $key, 'class="active"'); ?>>
										<a href="<?= site_url("statistik_web/chart_gis_rw/$key/" . underscore($rw[dusun]) . "/" . underscore($rw[rw])); ?>" data-remote="false" data-toggle="modal" data-target="#modalBesar" data-title="Statistik Penduduk RW <?= set_ucwords($rw['rw']) . " " . set_ucwords($wilayah) . set_ucwords($rw['dusun']); ?>">
											<?= $value ?>
										</a>
									</li>
								<?php endforeach; ?>
