							...
							<ul class="dropdown-menu">
								<?php foreach ($list_lap_front as $key => $value) : ?>
									<li <?= jecho($lap, $key, 'class="active"'); ?>>
										<a href="<?= site_url("statistik_web/chart_gis_rt/$key/" . underscore($rt[dusun]) . "/" . underscore($rt[rw]) . "/" . underscore($rt[rt])) ?>" data-remote="false" data-toggle="modal" data-target="#modalBesar" data-title="Statistik Penduduk RT <?= $rt['rt'] ?> RW <?= $rt['rw'] ?> <?= set_ucwords($wilayah) . " " . set_ucwords($rt['dusun']); ?>">
											<?= $value ?>
										</a>
									</li>
								<?php endforeach; ?>
