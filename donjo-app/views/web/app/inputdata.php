				...
				<?php if($list_sawah) { ?>
					...
					<div class="box-body">
						<div class="form-group form-row">
							...
							<div class="col-sm-7">
								<input type="text" name="tanggal_produksi" class="form-control form-control-sm datepicker required" value="<?= tgl_indo_out($produksi['tanggal_produksi']) ?>">
							</div>
						</div>
						<div class='form-group form-row'>
							<label class="control-label col-sm-3"><?= ucwords($this->setting->sebutan_dusun) ?></label>
							<div class="col-sm-7">
								<select name="dusun" class="form-control form-control-sm required select-dusun">
						...
						<div class="form-group form-row">
							...
							<div class="col-sm-7">
								<select name="sawah" class="form-control form-control-sm required select-sawah" id="idsawah">
									<option value="">Pilih Sawah</option>
									<?php foreach ($list_sawah as $data) : ?>
										<option value="<?= $data['id'] ?>" <?php selected($produksi['sawah'], $data['id']) ?>><?= $data['pemilik'] ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
						<div class="form-group form-row">
							...
							<div class="col-sm-7">
								<input name="harga" class="form-control form-control-sm required" maxlength="100" type="number" value="<?= $produksi['harga'] ?>"></input>
							</div>
						</div>
						<div class="form-group form-row">
							<label class="control-label col-sm-3">Jumlah Input (Kg)</label>
							<div class="col-sm-7">
								<input name="jumlah_panen" class="form-control form-control-sm required" maxlength="100" type="text" value="<?= $produksi['jumlah_panen'] ?>" pattern="^\d*(\.\d{0,2})?$"></input>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	$(function () {
        $('form').bind('submit', function () {
          $.ajax({
            type: 'post',
            url: '<?= base_url() ?>mobile_app/insert',
            data: $('form').serialize(),
            success: function () {
