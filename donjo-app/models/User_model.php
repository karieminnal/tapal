...
class User_model extends CI_Model
{
	...
	public function siteman()
	{
		...
		// Login sukses: ubah pass di db ke bcrypt jika masih md5 dan set session
			...
			if (($row->id_grup == self::GROUP_REDAKSI) && ($this->setting->offline_mode >= 2)) {
				...
				$_SESSION['desa'] = $row->id_desa;
				$_SESSION['filterDesa'] = $row->id_desa;
	...
	public function login()
	{
		...
		$sql = "SELECT id, password, id_grup, id_desa, session FROM user WHERE id_grup = 1 LIMIT 1";
	...
	public function update_password($id = 0)
	{
		...
		if (!empty($this->input->post('nama'))) $data['nama'] = alfanumerik_spasi($this->input->post('nama'));
	...
	public function cek_pass_lama_blur($id)
	{
		$_SESSION['success'] = 1;
		$_SESSION['error_msg'] = '';
		...
		if ($this->input->post('pass_lama') != '') {
			...
			if (password_verify($password, $row->password) === FALSE) {
				$_SESSION['error_msg'] .= 'Kata sandi lama salah';
			}
	...
	public function hak_akses($group, $controller, $akses)
	{
		...
		$hak_akses = array(
			...
			2 => array(
				...
				// leuit
				'leuit_analisa' => array('b', 'u', 'h'),
				'leuit_distribusi' => array('b', 'u', 'h'),
				'leuit_panen' => array('b', 'u', 'h'),
				'leuit_sawah' => array('b', 'u', 'h'),
				'leuit_lokasi' => array('b', 'u', 'h'),
