<?php class Penduduk_log_model extends CI_Model {
	...
	private function list_data_sql()
	{
		...
		AND log.id_detail IN (2,3,4,5,8)
		";
	...
	public function log_bulanan()
	{
		$bln = $this->db->escape($_SESSION['bulanku']);
		$thn = $this->db->escape($_SESSION['tahunku']);
		$sql = "SELECT l.tgl_peristiwa AS tgl, p.nama AS nama, p.nik AS nik, CONCAT('00',c.rt,'/00',c.rw) AS 'rt_rw', concat(l.id_detail,p.sex) AS detail
				FROM tweb_penduduk p
				LEFT JOIN tweb_wil_clusterdesa c ON p.id_cluster = c.id
				LEFT JOIN log_penduduk l ON p.id = l.id_pend
				WHERE l.bulan = $bln AND l.tahun = $thn
				";
		$query = $this->db->query($sql);
		$data = $query->result_array();
		return $data;
