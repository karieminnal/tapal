...
*/
	...
	protected function tulis_tweb_wil_clusterdesa(&$isi_baris)
	{
		...
		$query = "SELECT id FROM tweb_wil_clusterdesa WHERE dusun = ? AND id_desa = $filterDesa";
		$hasil = $this->db->query($query, $isi_baris['dusun']);
		$res = $hasil->row_array();
		if (empty($res))
		{
			$query = "INSERT INTO tweb_wil_clusterdesa(rt, rw, dusun, id_desa) VALUES (0, 0, '".$isi_baris['dusun']."', $filterDesa)";
			$hasil = $this->db->query($query);
			$query = "INSERT INTO tweb_wil_clusterdesa(rt, rw, dusun, id_desa) VALUES (0, '-', '".$isi_baris['dusun']."', $filterDesa)";
			$hasil = $this->db->query($query);
			$query = "INSERT INTO tweb_wil_clusterdesa(rt, rw, dusun, id_desa) VALUES ('-','-','".$isi_baris['dusun']."', $filterDesa)";
			$hasil = $this->db->query($query);
		}

		// --- Masukkan rw apabila belum ada
		$query = "SELECT id FROM tweb_wil_clusterdesa WHERE dusun = ? AND rw = ? AND id_desa = $filterDesa";
		$hasil = $this->db->query($query, array($isi_baris['dusun'], $isi_baris['rw']));
		$res = $hasil->row_array();
		if (empty($res))
		{
			$query = "INSERT INTO tweb_wil_clusterdesa(rt,rw,dusun, id_desa) VALUES (0, '".$isi_baris['rw']."', '".$isi_baris['dusun']."', $filterDesa)";
			$hasil = $this->db->query($query);
			$query = "INSERT INTO tweb_wil_clusterdesa(rt,rw,dusun, id_desa) VALUES ('-', '".$isi_baris['rw']."', '".$isi_baris['dusun']."', $filterDesa)";
			$hasil = $this->db->query($query);
			$isi_baris['id_cluster'] = $this->db->insert_id();
		}

		// --- Masukkan rt apabila belum ada
		$query = "SELECT id FROM tweb_wil_clusterdesa 
			WHERE dusun = '".$isi_baris['dusun']."' AND rw='".$isi_baris['rw']."' AND rt='".$isi_baris['rt']."' AND id_desa = '".$filterDesa."'";
		$hasil = $this->db->query($query);
		$res = $hasil->row_array();
		if ( ! empty($res))
		{
		...
		{
			$query = "INSERT INTO tweb_wil_clusterdesa(rt,rw,dusun, id_desa) VALUES ('".$isi_baris['rt']."', '".$isi_baris['rw']."', '".$isi_baris['dusun']."', $filterDesa)";
			$hasil = $this->db->query($query);
			$isi_baris['id_cluster'] = $this->db->insert_id();
		}
	}
	...
	public function import_excel($hapus=false)
	{
		...
		$htmlprogressFooter = '</body></html>';
		$htmlprogressBody = '<div class="row">
			<div class="col-lg-2"></div>
				<div class="col-lg-8">
					<div class="progress-container">
						<h4 class="text-center mb-5">
							Import file <b>'.$_FILES["userfile"]["name"].'</b>. <br>Jangan menutup tab ini!
						</h4>
						<div class="progress" style="height:20px">
							<div class="progress-bar progress-bar-striped progress-bar-animated" id="progress-bar" style="height:20px;padding:0px">
								<span id="progess-info"></span> 
							</div>
						</div>
						<div class="data-sukses"></div>
						<div class="data-gagal">Gagal: -</div>
						<div class="baris-gagal"></div>
						<br>
						<div class="text-center">
							<a href="/database/import/1">Kembali</a>
						</div>
					</div>
				</div>
			<div class="col-lg-2"></div>
		</div>';
		echo $htmlprogressHead.$htmlprogressBody.$htmlprogressFooter;
		$gagal = 0;
		$baris_gagal = "";
		$baris_kosong = 0;
		...
		for ($i=2; $i<=$baris; $i++)
		{
			...
			$barisXls = $baris-1;
			$k = $i-1;
			$percent = intval($k/$barisXls * 100)."%";
			$progresfile = '<script type="text/javascript">
				$("#progress-bar").css("width","'.$percent.'");
				$("#progess-info").html("'.($i-1).' record ('.$percent.')");
				$("head > title").html("'.($i-1).' record ('.$percent.')");
				//$(".data-sukses").html("Sukses: <b>'.($baris_data - $baris_kosong - $gagal - 1).'</b>");
				$(".data-gagal").html("Gagal: <b>'.$gagal.'</b>");
				// var barisGagal = "<div>'.$baris_gagal.'</div>";
				// $("<div>'.$baris_gagal.'</div>").appendTo(".baris-gagal");
			</script>';

			echo $progresfile;
			flush();
			// gunakan fungsi usleep 0,25 detik (25000 milisecond) untuk membuat jeda
			usleep(25000);
		}
