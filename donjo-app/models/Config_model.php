...
class Config_model extends CI_Model
{
	...
	public function penduduk_total($idesa)
	{
		$sql = "SELECT COUNT(p.id) AS jumlah FROM tweb_penduduk p 
		LEFT JOIN tweb_wil_clusterdesa c ON p.id_cluster = c.id 
		WHERE status_dasar = 1 AND c.id_desa = '$idesa'";
		$query = $this->db->query($sql);
		$data = $query->result_array();
		return $data;
	}

	public function get_provinsi_global(){
		$query = $this->db->select('*')->limit(1)->get('config_provinsi')->row_array();
		return $query;
	}
	...
	public function get_data_all(){
		$sql = 'SELECT * FROM config ORDER BY nama_kabupaten ASC, nama_desa ASC';
		$query = $this->db->query($sql);
		$data = $query->result_array();
		return $data;
	}
	...
	public function get_data_all_leuit($year=''){
		if($year) {
			...
			$tahunDis = " AND YEAR(leuit_distribusi.tanggal_distribusi) = $year";
		} else {
			$tahunProd = "";
			$tahunDis = "";
		}
		$sql = "SELECT c.*, 
		(SELECT SUM(leuit_produksi.jumlah_panen) FROM leuit_produksi WHERE (leuit_produksi.id_desa = c.id) $tahunProd) AS total_input, 
		(SELECT SUM(leuit_distribusi.jumlah_distribusi) FROM leuit_distribusi WHERE (leuit_distribusi.id_desa = c.id) $tahunDis) AS total_output, 
		(SELECT SUM(leuit_distribusi.jumlah_distribusi) FROM leuit_distribusi WHERE (leuit_distribusi.id_desa = c.id) AND leuit_distribusi.jenis = 'Komersil' $tahunDis) AS komersil, 
		(SELECT SUM(leuit_distribusi.jumlah_distribusi) FROM leuit_distribusi WHERE (leuit_distribusi.id_desa = c.id) AND leuit_distribusi.jenis = 'Produksi' $tahunDis) AS produksi, 
		(SELECT SUM(leuit_distribusi.jumlah_distribusi) FROM leuit_distribusi WHERE (leuit_distribusi.id_desa = c.id) AND leuit_distribusi.jenis = 'Distribusi/Logistik' $tahunDis) AS logistik 
		FROM config c
	...
	*/
	...
	private function post_provinsi()
	{
		$filterDesa = $_SESSION['filterDesa'];
		$post = $this->input->post();
		$data['old_logo'] = $post['old_logo'];
	...
	public function updateProvinsi()
	{
		...
		// $data = $this->post_provinsi();
		$post = $this->input->post();
		$data['nama_provinsi'] = $post['nama_propinsi'];
		$data['id_provinsi'] = $post['pilihProvinsi'];
		$data['id_kota'] = $post['pilihKota'];
		$data['id_kecamatan'] = $post['pilihKecamatan'];
		$data['id_kelurahan'] = $post['pilihKelurahan'];
		$data['kode_pos'] = $post['kode_pos'];
		$data['nama_jalan'] = $post['nama_jalan'];
		$data['gubernur'] = $post['gubernur'];
		$data['lat'] = $post['lat'];
		$data['lng'] = $post['lng'];

		$this->db->where('id', $id);
		$outp = $this->db->update('config_provinsi', $data);
