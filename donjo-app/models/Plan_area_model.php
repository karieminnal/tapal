...
class Plan_area_model extends MY_Model {
	...
	public function list_data($o=0,$offset=0, $limit=1000)
	{
		...
		$sql = "SELECT l.*,p.nama AS kategori,m.nama AS jenis,p.simbol AS simbol,p.color AS color
			, c.rt AS rt, c.rw AS rw, c.dusun AS dusun
			FROM area l
	...
	{
		$sql = "SELECT l.* , c.rt AS rt, c.rw AS rw, c.dusun AS dusun FROM area l LEFT JOIN tweb_wil_clusterdesa c ON l.id_cluster = c.id WHERE l.id = ?";
