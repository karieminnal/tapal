...
class Laporan_penduduk_model extends MY_Model
{
	...
	{
		$sql = "(SELECT COUNT(p.id) FROM penduduk_hidup p
	...
	public function judul_statistik($lap)
	{
		...
		switch ("$lap") {
			case "kelas_sosial":
				return "Klasifikasi Sosial";
				break;
			case "0":
				return "Pendidikan Dalam KK";
				break;
			case "1":
				return "Pekerjaan";
				break;
			...
			case "9":
				...
				break;
			case "10":
				return "Sakit Menahun";
				break;
			case "13":
				return "Rentang Umur";
				break;
			case "14":
				return "Pendidikan Sedang Ditempuh";
				break;
			case "15":
				return "Kategori Umur";
				break;
			case "16":
				return "Akseptor KB";
				break;
			...
			case "bantuan_penduduk":
				return "Penerima Bantuan (Penduduk)";
				break;
			case "bantuan_keluarga":
				return "Penerima Bantuan (Keluarga)";
				break;
			default:
				return NULL;
		}
	...
	private function select_jml_penduduk_per_kategori($id_referensi, $tabel_referensi, $desaid)
	{
		$this->db
	...
	protected function data_jml_semua_penduduk($desaid)
	{
		$this->db
	...
	private function select_jml($where, $desaid)
	{
		$str_jml_penduduk = $this->str_jml_penduduk($where,'', $desaid);
		$str_jml_laki = $this->str_jml_penduduk($where, '1', $desaid);
		$str_jml_perempuan = $this->str_jml_penduduk($where, '2', $desaid);
		$this->db
	...
	private function str_jml_penduduk($where, $sex = '', $desaid)
	{
		if ($dusun = $this->session->userdata("dusun")) $this->db->where('a.dusun', $dusun);
		if ($rw = $this->session->userdata("rw")) $this->db->where('a.rw', $rw);
		if ($rt = $this->session->userdata("rt")) $this->db->where('a.rt', $rt);
		if ($sex) $this->db->where('p.sex', $sex);
		$this->db->where("a.id_desa", $desaid);
		$str_jml_penduduk = $this->db->select('COUNT(p.id)')
			->from('penduduk_hidup p')
	...
	protected function select_per_kategori($desaid = 0)
	{
		$lap = $this->lap;
		...
		switch ("$lap") {
			...
			case in_array($lap, array_keys($statistik_penduduk)):
				$this->select_jml_penduduk_per_kategori($statistik_penduduk["$lap"]['id_referensi'], $statistik_penduduk["$lap"]['tabel_referensi'], $desaid);
				break;
			...
			default:
				$this->select_jml_penduduk_per_kategori($statistik_penduduk["0"]['id_referensi'], $statistik_penduduk["0"]['tabel_referensi'], $desaid);
		}
	}

	protected function get_data_jml($desaid)
	{
		$lap = $this->lap;
		...
		if ($lap == 18) {
			...
			$semua = $this->data_jml_semua_penduduk($desaid);
	...
	public function list_data($lap = 0, $o = 0, $desaid)
	{ 
		$this->lap = $lap;
		...
		$this->db->where("a.id_desa", $desaid);
		$namespace->select_per_kategori($desaid);
		$this->order_by($o);
		$data = $this->db->get()->result_array();
		$this->isi_nomor($data);

		$semua = $namespace->get_data_jml($desaid);
	...
	public function list_data_rentang_api()
	{
		$query = $this->db
			->where('status', 0)
			->order_by('dari')
