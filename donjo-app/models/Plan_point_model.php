...
class Plan_point_model extends MY_Model
{
	...
	public function list_data($o = 0, $offset = 0, $limit = 1000)
	{
		switch ($o) {
			...
			case 4:
				...
				break;
			case 5:
				$order_sql = ' ORDER BY urutan';
				break;
			case 6:
				$order_sql = ' ORDER BY urutan DESC';
				break;
			default:
				$order_sql = ' ORDER BY urutan ASC';
		}
	...
	public function list_data_menu_gis()
	{

		$sql   = "SELECT * FROM point WHERE tipe = 0 AND enabled = 1 ORDER BY urutan ASC";
		$query = $this->db->query($sql);
		$data = $query->result_array();
		return $data;
	}
	...
	{
		$sql = "SELECT * FROM point WHERE parrent = ? AND tipe = 2 ORDER BY urutan";
	...
	// public function get_lokasi_kategori($point = 1)
	// {
	// 	$this->db->select('*')
	// 		->from('point')
	// 		->where('type', '2')
	...
	public function get_lokasi_kategori($point = 1)
	{
		...
		// if (!empty($jenis)) $sql .= ' AND parrent = ' . $jenis;
		$query = $this->db->query($sql, $point);
		$data = $query->result_array();
		return $data;
	}
