...
class First_artikel_m extends CI_Model
{
	...
	public function artikelMobile($offset, $limit)
	{
		$this->paging_artikel_sql();
		$data = $this->db
			->select('a.*, u.nama AS owner, k.kategori, k.tipe AS tipe, k.slug AS kat_slug, YEAR(tgl_upload) as thn, MONTH(tgl_upload) as bln, DAY(tgl_upload) as hri')
			->where('k.tipe', "1")
			->where('a.enabled', "1")
