...
class User_setting extends Admin_Controller
{
	...
	public function update_password($id = '')
	{
		...
		if ($this->session->from_first) {
			if ($this->session->success == -1) {
				echo '<div class="alert alert-danger">' . $this->session->error_msg . '</div>';
			} else {
	...
	public function check_pass_lama($id = '')
	{
		$this->user_model->cek_pass_lama_blur($id);
		if ($this->session->success == -1) {
			echo $this->session->error_msg;
		} else {
			echo 1;
		}
	}
