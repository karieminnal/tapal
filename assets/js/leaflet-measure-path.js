...
function checkUkuran() {
  var thisCheck = $('.lahan-check-container input[type="checkbox"]');
    ...
    updateMeasurements: function() {
      ...
      if (latLngs.length > 1) {
        for (
          ...
          // Show segment length
          if (
            options.showSegmentLength &&
            pixelDist >= options.minPixelDistance
          ) {
            L.marker
              .measurement(
        ...
        if (options.showTotalLength && !isPolygon) {
          L.marker
            .measurement(
              ll2,
              options.prefix.totalLength + distFormatter(totalDist),
      ...
      if (isPolygon && latLngs.length > 2) {
        ...
        if (options.showTotalArea) {
          ...
          L.marker
            .measurement(
              ...
              options.prefix.totalArea + areaFormatter(area),
        ...
        if (options.showTotalAreaLength) {
          ...
          L.marker
            .measurement(
              ll2,
              options.prefix.totalAreaLength + distFormatter(totalDist),
              options.lang.totalAreaLength,
              0,
              options,
            )
            .addTo(this._measurementLayer);
